...
import { db, rechtsformen, gewerbeanmeldungInfo, taxObligations, taxDeadlines, permits, berufsgenossenschaften, svContributionRates, svObligations, hrObligations, notaryCosts, fundingPrograms, } from "../db.js";
...
const TOOLS = [
    {
        ...
        description: "Fetch German legal entity types (Rechtsformen) from the database. Returns comparison data including minimum capital, liability type, notary requirements, and founding costs.",
        input_schema: {
            type: "object",
            properties: {
                slug: {
    ...
    {
        name: "get_steuern",
        description: "Fetch German tax obligations (Steuerliche Pflichten) and filing deadlines for founders. Can filter by Rechtsform slug.",
        input_schema: {
            type: "object",
            properties: {
                rechtsformSlug: {
    ...
    },
    {
        ...
        description: "Fetch business registration (Gewerbeanmeldung) requirements per Bundesland: costs, processing time, required documents, and online availability.",
        input_schema: {
            type: "object",
            properties: {
                bundesland: {
    ...
    },
    {
        ...
        description: "Fetch required permits and licences (Genehmigungen) for specific trades, plus Berufsgenossenschaft (statutory accident insurance) information.",
        input_schema: {
            type: "object",
            properties: {
                tradeCategory: {
                ...
                permitCategory: {
                    type: "string",
                    description: "Optional: permit category filter (e.g. 'erlaubnispflichtiges_gewerbe', 'meisterpflicht').",
                },
                query: {
                    type: "string",
    ...
    },
    {
        ...
        description: "Fetch social insurance contribution rates (Beitragssätze) and employer obligations (Meldepflichten) for German founders hiring employees.",
        input_schema: {
            type: "object",
            properties: {},
            required: [],
    ...
    },
    {
        ...
        description: "Fetch trade register (Handelsregister) obligations and notary costs for different Rechtsformen.",
        input_schema: {
            type: "object",
            properties: {
                rechtsformSlug: {
                    type: "string",
                    description: "Optional: filter by Rechtsform slug (e.g. 'gmbh', 'ag'). Omit for all.",
                },
            },
            required: [],
        },
    },
    {
        ...
        description: "Search and filter German funding programs (Förderprogramme) for startups: grants, loans, and guarantees from federal, state, and EU sources.",
        input_schema: {
            type: "object",
            properties: {
                query: {
    ...
    {
        name: "search_sophex",
        description: "Cross-silo full-text search across all Sophex data: Rechtsformen, taxes, permits, social insurance, Handelsregister, and funding programs.",
        input_schema: {
            type: "object",
            properties: {
                query: {
...
async function handleGetRechtsformen(input) {
...
async function handleGetGewerbeanmeldung(input) {
...
async function handleGetGenehmigungen(input) {
    ...
    const conditions = [];
    if (input.tradeCategory)
        conditions.push(eq(permits.tradeCategory, input.tradeCategory));
    if (input.permitCategory)
        conditions.push(eq(permits.permitCategory, input.permitCategory));
    if (input.query)
        conditions.push(ilike(permits.labelDe, `%${input.query}%`));
...
async function handleGetSozialversicherung() {
...
async function handleGetHandelsregister(input) {
    const where = input.rechtsformSlug
        ? or(eq(hrObligations.rechtsformSlug, input.rechtsformSlug), eq(hrObligations.rechtsformSlug, "all"))
        : undefined;
...
async function handleGetFoerderprogramme(input) {
    ...
    if (input.level)
        conditions.push(eq(fundingPrograms.level, input.level));
    if (input.state)
        conditions.push(ilike(fundingPrograms.state, `%${input.state}%`));
    if (input.fundingType)
        conditions.push(ilike(fundingPrograms.fundingType, `%${input.fundingType}%`));
...
async function handleSearchSophex(input) {
    const q = input.query;
    const [rechtsformResults, permitResults, fundingResults, taxResults] = await Promise.all([
        db
            .select({ id: rechtsformen.id, name: rechtsformen.name, sourceUrl: rechtsformen.sourceUrl, type: sql `'rechtsform'` })
            .from(rechtsformen)
            .where(or(ilike(rechtsformen.name, `%${q}%`), ilike(rechtsformen.fullName, `%${q}%`)))
            .limit(5),
        db
            .select({ id: permits.id, name: permits.labelDe, sourceUrl: permits.sourceUrl, type: sql `'permit'` })
            .from(permits)
            .where(or(ilike(permits.labelDe, `%${q}%`), ilike(permits.descriptionDe, `%${q}%`)))
            .limit(5),
        db
            .select({ id: fundingPrograms.id, name: fundingPrograms.titleDe, sourceUrl: fundingPrograms.sourceUrl, type: sql `'funding'` })
            .from(fundingPrograms)
            .where(and(eq(fundingPrograms.isActive, true), sql `search_vector @@ plainto_tsquery('german', ${q})`))
            .limit(5),
        db
            .select({ id: taxObligations.id, name: taxObligations.taxType, sourceUrl: taxObligations.sourceUrl, type: sql `'tax'` })
            .from(taxObligations)
...
async function executeTool(name, input) {
    switch (name) {
        case "get_rechtsformen":
            return handleGetRechtsformen(input);
        case "get_steuern":
            return handleGetSteuern(input);
        case "get_gewerbeanmeldung":
            return handleGetGewerbeanmeldung(input);
        case "get_genehmigungen":
            return handleGetGenehmigungen(input);
        case "get_sozialversicherung":
            return handleGetSozialversicherung();
        case "get_handelsregister":
            return handleGetHandelsregister(input);
        case "get_foerderprogramme":
            return handleGetFoerderprogramme(input);
        case "search_sophex":
            return handleSearchSophex(input);
        default:
...
assistantRouter.post("/", async (c) => {
    ...
    if (!apiKey) {
        return c.json({ data: null, meta: null, error: "AI assistant is not configured (missing ANTHROPIC_API_KEY)" }, 503);
    }
    // Rate limiting: 10 req/min per IP
    const ip = c.req.header("x-forwarded-for")?.split(",")[0]?.trim() ?? "unknown";
    if (!checkRateLimit(ip)) {
        return c.json({ data: null, meta: null, error: "Rate limit exceeded: max 10 requests per minute" }, 429);
    }
    // Parse and validate request body
    const body = await c.req.json().catch(() => null);
    const parsed = requestSchema.safeParse(body);
    if (!parsed.success) {
        return c.json({ data: null, meta: null, error: "Invalid request", details: parsed.error.flatten() }, 400);
    }
    ...
    if (context?.bundesland || context?.rechtsform) {
        const ctxParts = [];
        if (context.bundesland)
            ctxParts.push(`Bundesland: ${context.bundesland}`);
        if (context.rechtsform)
            ctxParts.push(`Rechtsform: ${context.rechtsform}`);
