...
steuernRouter.get("/steuern", async (c) => {
    ...
    if (rechtsform) {
        ...
        conditions.push(or(eq(taxObligations.rechtsformSlug, rechtsform), eq(taxObligations.rechtsformSlug, "all")));
    }
    if (taxType) {
...
steuernRouter.get("/steuern/rechtsform/:slug", async (c) => {
    ...
    const rows = await db
        .select()
        .from(taxObligations)
        .where(or(eq(taxObligations.rechtsformSlug, slug), eq(taxObligations.rechtsformSlug, "all")))
        .orderBy(asc(taxObligations.taxType));
    if (rows.length === 0) {
        return c.json({ data: null, meta: null, error: `No tax obligations found for Rechtsform: ${slug}` }, 404);
    }
    return c.json({
        data: rows,
...
steuernRouter.get("/steuern/kleinunternehmer", async (c) => {
    const rows = await db
        .select()
        .from(taxObligations)
        .where(or(eq(taxObligations.taxType, "kleinunternehmerregelung"), eq(taxObligations.kleinunternehmerRelevant, true)))
        .orderBy(asc(taxObligations.taxType));
...
//# sourceMappingURL=steuern.js.map