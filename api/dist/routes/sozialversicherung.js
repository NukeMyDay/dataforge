...
sozialversicherungRouter.get("/sozialversicherung/minijob", async (c) => {
    const [rateRows, obligationRows] = await Promise.all([
        db
            .select()
            .from(svContributionRates)
            .where(ilike(svContributionRates.insuranceType, "minijob%"))
            .orderBy(asc(svContributionRates.insuranceType)),
        db
            .select()
            .from(svObligations)
            .where(or(eq(svObligations.obligationType, "minijob_regelungen"), ilike(svObligations.appliesTo, "%minijobber%")))
            .orderBy(asc(svObligations.obligationType)),
    ]);
    return c.json({
        data: {
...
//# sourceMappingURL=sozialversicherung.js.map