...
institutionsRouter.get("/", async (c) => {
    ...
    const conditions = [];
    if (country)
        conditions.push(eq(institutions.country, country));
    if (type)
        conditions.push(eq(institutions.type, type));
    if (q) {
        const pattern = `%${q}%`;
        conditions.push(or(ilike(institutions.nameDe, pattern), ilike(institutions.nameEn, pattern), ilike(institutions.nameNl, pattern)));
    }
...
institutionsRouter.get("/:id", async (c) => {
    ...
    const rows = await db
        ...
        programCount: sql `cast(count(${programs.id}) as integer)`,
    })
        .from(institutions)
