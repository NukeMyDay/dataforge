...
rechtsformenRouter.get("/rechtsformen", async (c) => {
    ...
    const [rows, countResult] = await Promise.all([
        db
            ...
            .limit(pageSize)
            .offset(offset),
        db.select({ total: sql `count(*)::int` }).from(rechtsformen),
    ]);
...
rechtsformenRouter.get("/rechtsformen/:slug", async (c) => {
    ...
    const rows = await db
        ...
        .limit(1);
    if (rows.length === 0) {
        return c.json({ data: null, meta: null, error: "Rechtsform not found" }, 404);
    }
...
rechtsformenRouter.get("/gewerbeanmeldung", async (c) => {
    ...
    const [rows, countResult] = await Promise.all([
        db
            ...
            .limit(pageSize)
            .offset(offset),
        db.select({ total: sql `count(*)::int` }).from(gewerbeanmeldungInfo),
    ]);
    const countRows = countResult[0]?.total ?? 0;
    return c.json({
        data: rows,
...
rechtsformenRouter.get("/gewerbeanmeldung/:bundesland", async (c) => {
    ...
    const rows = await db
        ...
        .limit(1);
    if (rows.length === 0) {
        return c.json({ data: null, meta: null, error: "Bundesland not found" }, 404);
    }
    return c.json({ data: rows[0], meta: null, error: null });
});
//# sourceMappingURL=rechtsformen.js.map