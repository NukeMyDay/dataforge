 ...
 */
export async function sendJob(jobName: string, data: Record<string, unknown> = {}): Promise<string> {
  const result = await db.execute(
    sql`INSERT INTO pgboss.job (name, data, state, startafter, expirein, keepuntil)
        VALUES (
          ${jobName},
