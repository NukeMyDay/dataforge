...
import { db, pipelines, pipelineRuns, apiKeys, users, settings, programs, institutions, regulations } from "../db.js";
...
adminRouter.get("/data-quality", async (c) => {
  ...
  function completeness(missing: number, total: number) {
    if (total === 0) return 100;
    return Math.round(((total - missing) / total) * 100);
  }

  const ps = programStats as Record<string, number>;
  const is = institutionStats as Record<string, number>;
  const rs = regulationStats as Record<string, number>;
