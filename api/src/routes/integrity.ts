...
integrityRouter.get("/:recordType/:recordId", async (c) => {
  ...
  const proofRows = await db
    ...
    .where(
      and(
        eq(anchorProofs.recordType, recordType),
        eq(anchorProofs.recordId, recordId),
      ),
    )
  ...
  const proofs = proofRows.map((row) => {
    const merklePath = row.merklePath as MerkleProofStep[];
  ...
  return c.json({
    data: {
      ...
      verificationInstructions: buildVerificationInstructions(
        latestProof.contentHash,
        latestProof.merklePath,
        latestProof.anchor.merkleRoot,
        latestProof.anchor.network,
        latestProof.anchor.txId,
      ),
    },
    meta: {
      recordType,
...
function buildExplorerUrl(network: string, txId: string | null): string | null {
  if (!txId) return null;
  switch (network) {
    case "ethereum":
...
function buildVerificationInstructions(
  ...
  const steps = [
    ...
    ...merklePath.map(
      (step, i) =>
        `   Step ${i + 1}: combine current hash with ${step.position} sibling ${step.hash}`,
    ),
  ...
  if (network === "simulated") {
    steps.push(
      `3. This anchor uses a SIMULATED network (PoC only). It is not externally verifiable.`,
      `   In production, switch ANCHOR_NETWORK=opentimestamps for BTC-backed proofs.`,
    );
  } else if (network === "opentimestamps") {
    steps.push(
      `3. The Merkle root was submitted to the OpenTimestamps BTC calendar.`,
      txId
        ? `   Submission ID: ${txId}`
        : `   Submission is pending confirmation.`,
      `   To verify: use the opentimestamps-client CLI with the stored OTS proof file.`,
      `   See: https://opentimestamps.org`,
    );
  } else if (network === "ethereum" && txId) {
    steps.push(
      `3. Look up transaction ${txId} on Ethereum.`,
      `   Verify the tx calldata contains the Merkle root: ${merkleRoot}`,
      `   Explorer: https://etherscan.io/tx/${txId}`,
    );
  } else if (network === "bitcoin" && txId) {
    steps.push(
      `3. Look up transaction ${txId} on Bitcoin.`,
      `   Verify the OP_RETURN output contains the Merkle root: ${merkleRoot}`,
