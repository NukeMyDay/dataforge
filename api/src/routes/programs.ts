...
programsRouter.get("/", async (c) => {
  ...
  const { page, pageSize, country, degreeType, fieldOfStudy, language, institutionId, isActive, q, sort } =
    parsed.data;

  const conditions = [];

  if (country) conditions.push(eq(programs.country, country));
  if (degreeType) conditions.push(eq(programs.degreeType, degreeType));
  if (fieldOfStudy) conditions.push(eq(programs.fieldOfStudy, fieldOfStudy));
  if (language) conditions.push(eq(programs.language, language));
  if (institutionId) conditions.push(eq(programs.institutionId, institutionId));
  if (isActive !== undefined) conditions.push(eq(programs.isActive, isActive === "true"));
