    ...
    (silo === "all" || silo === "programs")
      ? db
          .select({
            id: programs.id,
            slug: programs.slug,
            title: sql<string>`coalesce(${programs.titleEn}, ${programs.titleNl}, ${programs.titleDe})`,
            subtitle: sql<string>`coalesce(${programs.degreeType}, '')`,
            country: programs.country,
            type: sql<string>`'program'`,
            rank: sql<number>`ts_rank(programs.search_vector, plainto_tsquery('simple', ${q}))`,
          })
          .from(programs)
          .where(
            and(
              sql`programs.search_vector @@ plainto_tsquery('simple', ${q})`,
              country ? eq(programs.country, country) : undefined,
            )
          )
    ...
    (silo === "all" || silo === "institutions")
      ? db
          .select({
            ...
            title: sql<string>`coalesce(${institutions.nameEn}, ${institutions.nameNl}, ${institutions.nameDe})`,
            subtitle: sql<string>`coalesce(${institutions.city}, '')`,
            country: institutions.country,
            type: sql<string>`'institution'`,
            rank: sql<number>`ts_rank(institutions.search_vector, plainto_tsquery('simple', ${q}))`,
          })
          .from(institutions)
          .where(
            and(
              sql`institutions.search_vector @@ plainto_tsquery('simple', ${q})`,
              country ? eq(institutions.country, country) : undefined,
            )
          )
    ...
    (silo === "all" || silo === "regulations")
      ? db
          .select({
            ...
            title: sql<string>`coalesce(${regulations.titleEn}, ${regulations.titleDe})`,
            subtitle: sql<string>`coalesce(${regulations.jurisdiction}, '')`,
            country: sql<string>`'DE'`,
            type: sql<string>`'regulation'`,
            rank: sql<number>`ts_rank(regulations.search_vector, plainto_tsquery('simple', ${q}))`,
          })
          .from(regulations)
