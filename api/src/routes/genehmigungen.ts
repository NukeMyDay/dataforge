...
genehmigungenRouter.get("/genehmigungen", async (c) => {
  ...
  const conditions = [];
  if (tradeCategory) conditions.push(eq(permits.tradeCategory, tradeCategory));
  if (permitCategory) conditions.push(eq(permits.permitCategory, permitCategory));
  if (q) conditions.push(ilike(permits.labelDe, `%${q}%`));
...
genehmigungenRouter.get("/genehmigungen/:id", async (c) => {
  ...
  if (row.length === 0) {
    return c.json(
      { data: null, meta: null, error: `Permit not found: ${idOrKey}` },
      404,
    );
  }
