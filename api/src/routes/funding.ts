...
fundingRouter.get("/", async (c) => {
  ...
  if (region) conditions.push(ilike(fundingPrograms.fundingRegion, `%${region}%`));
  if (type) conditions.push(ilike(fundingPrograms.fundingType, `%${type}%`));
  if (targetGroup) conditions.push(ilike(fundingPrograms.eligibleApplicants, `%${targetGroup}%`));
  if (level) conditions.push(eq(fundingPrograms.level, level));
  if (state) conditions.push(ilike(fundingPrograms.state, `%${state}%`));
