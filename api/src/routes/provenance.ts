 ...
 */
function computeConfidence(p: ConfidenceParams): number {
  // Freshness score
  let freshness = 5; // default: never scraped or very stale
  if (p.lastScrapedAt) {
    const days = (Date.now() - p.lastScrapedAt.getTime()) / 86_400_000;
    if (days <= 7) freshness = 40;
    else if (days <= 14) freshness = 35;
    else if (days <= 30) freshness = 25;
    else if (days <= 90) freshness = 15;
  }
  ...
  if (p.checkCount > 2) {
    ...
    if (rate < 0.05) stability = 40;      // < 5% of checks showed a change
    else if (rate < 0.2) stability = 30;  // < 20%
    else if (rate < 0.5) stability = 20;  // < 50%
    else stability = 10;                  // frequently changing source
  }

  // Source reliability score
  let source = 10;
  if (p.sourceUrl.includes("foerderdatenbank.de")) source = 20; // official German government portal
