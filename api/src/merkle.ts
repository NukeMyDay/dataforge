...
export interface MerkleProofStep {
  hash: string;       // hex-encoded SHA-256 sibling hash
  position: "left" | "right";
}

export interface MerkleTree {
  root: string;                  // hex-encoded Merkle root
  leaves: string[];              // ordered leaf hashes (padded to even length)
  proofs: MerkleProofStep[][];   // proof[i] is the Merkle path for leaves[i]
}
