...
export interface RecordToAnchor {
  recordId: string;    // logical ID, e.g. "123"
  recordType: string;  // domain, e.g. "funding"
  contentJson: string; // canonical JSON serialisation of record content
}
 ...
 */
async function anchorSimulated(merkleRoot: string): Promise<{ txId: string; status: "confirmed" }> {
 ...
 */
async function anchorOpenTimestamps(
  merkleRoot: string,
): Promise<{ txId: string | null; status: "pending" | "confirmed"; proofData: string | null }> {
...
export async function batchAnchor(
  records: RecordToAnchor[],
  network: AnchorNetwork = (process.env["ANCHOR_NETWORK"] as AnchorNetwork) ?? "simulated",
 ...
 */
export async function getRecordProofs(recordType: string, recordId: string) {
  return db
    .select({
    ...
    .where(
      and(
        ...
        eq(anchorProofs.recordId, String(recordId)),
      ),
    )
