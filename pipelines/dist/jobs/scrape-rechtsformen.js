...
// ─── RechtsformenScraper ──────────────────────────────────────────────────────
class RechtsformenScraper extends BaseScraper {
    constructor() {
        super({
    ...
    parsePage(html, url) {
        ...
        let firstTable = searchRoot.find("table").filter((_, tbl) => {
            return $(tbl).find("th, td").toArray().some((cell) => $(cell).text().toLowerCase().includes("mindestkapital"));
        }).first();
        ...
        firstTable.find("tr").each((_, row) => {
            ...
            if (/keines|kein\s/i.test(capitalText) || capitalText === "") {
                ...
                if (m) {
                    ...
                    if (!isNaN(parsed))
                        minCapitalEur = Math.round(parsed);
                }
            }
            // cells[2]: Gründer minimum
            if (cells.length > 2)
                founderCount = $(cells[2]).text().trim().substring(0, 64) || null;
            // cells[3]: Haftung
            if (cells.length > 3)
                liabilityType = $(cells[3]).text().trim().substring(0, 256) || null;
        ...
        if (minCapitalEur === null) {
            const pageText = $.text();
            const capitalMatch = pageText.match(new RegExp(`${knownName}[^€EUR]*?(?:Mindest(?:stamm|grund|kapital)|Stammkapital|Grundkapital)[^\\d]*(\\d[\\d.,]*)\\s*(?:Euro|EUR)`, "i"));
            if (capitalMatch) {
...
class GewerbeanmeldungScraper extends BaseScraper {
    constructor() {
        super({
    ...
    parsePage(html, url) {
        ...
        let kostenEur = null;
        const feeMatch = pageText.match(/(?:Gebühr|Kosten|Entgelt|Verwaltungsgebühr)[^\d]{0,30}(\d{1,3})\s*(?:bis\s*\d+\s*)?(?:Euro|EUR)/i);
        if (feeMatch) {
        ...
        if (kostenEur === null)
            kostenEur = portal.baseKostenEur;
        // ── Processing time ───────────────────────────────────────────────────────
        let bearbeitungszeitTage = null;
        const timeMatch = pageText.match(/(?:Bearbeitungszeit|bearbeit)[^\d]{0,20}(\d+)\s*(?:bis\s*\d+\s*)?(?:Tag|Werktag|Arbeitstag)/i);
        if (timeMatch) {
        ...
        const pageSignalsOnline = onlineLower.includes("online beantragen") ||
        ...
        // ── Zuständige Stelle ─────────────────────────────────────────────────────
        const zustaendigeStelleDescription = extractSection($, ["zuständig", "zuständige behörde", "gewerbeamt", "ordnungsamt"]) ??
...
export async function scrapeRechtsformen() {
    const rechtsformenScraper = new RechtsformenScraper();
    const rechtsformenStats = await rechtsformenScraper.run();
    console.log(`[scrape-rechtsformen] Done — new: ${rechtsformenStats.newCount}, ` +
    ...
    console.log(`[scrape-gewerbeanmeldung] Done — new: ${gewerbeStats.newCount}, ` +
        `updated: ${gewerbeStats.updatedCount}, ` +
        `unchanged: ${gewerbeStats.unchangedCount}, ` +
        `errors: ${gewerbeStats.errorCount}`);
}
//# sourceMappingURL=scrape-rechtsformen.js.map