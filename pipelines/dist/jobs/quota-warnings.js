...
export async function runQuotaWarnings() {
    ...
    const nearLimitKeys = await db
        ...
        .from(apiKeys)
        .where(and(eq(apiKeys.tier, "free"), eq(apiKeys.isActive, true), isNotNull(apiKeys.userId), gt(apiKeys.requestCount, Math.floor(FREE_TIER_DAILY_LIMIT * QUOTA_WARN_THRESHOLD)), lt(apiKeys.requestCount, FREE_TIER_DAILY_LIMIT)))
        .limit(200);
    for (const key of nearLimitKeys) {
        ...
        await sendApiKeyQuotaWarningEmail(user.email, usedPercent).catch((e) => console.error(`[quota-warnings] quota email failed for ${user.email}:`, e));
    }
    ...
    const soonExpiring = await db
        ...
        .from(apiKeys)
        .where(and(eq(apiKeys.isActive, true), isNotNull(apiKeys.expiresAt), isNotNull(apiKeys.userId), gt(apiKeys.expiresAt, now), lt(apiKeys.expiresAt, new Date(now.getTime() + KEY_EXPIRY_WARN_DAYS * 24 * 60 * 60 * 1000))))
        .limit(200);
    for (const key of soonExpiring) {
        ...
        await sendApiKeyExpiryWarningEmail(user.email, key.name ?? `Key #${key.id}`, daysLeft).catch((e) => console.error(`[quota-warnings] expiry email failed for ${user.email}:`, e));
    }
    console.log(`[quota-warnings] Done — quota: ${nearLimitKeys.length}, expiry: ${soonExpiring.length}`);
}
//# sourceMappingURL=quota-warnings.js.map