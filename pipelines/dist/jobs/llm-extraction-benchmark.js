...
import { extractFieldsWithLLM } from "../lib/llm-extractor.js";
// ─── Config ──────────────────────────────────────────────────────────────────
 ...
 */
function simulateStructureChange(html) {
    return html
...
function fillMap(record) {
    return Object.fromEntries(TRACKED_FIELDS.map((f) => [f, record[f] !== null]));
}
// ─── URL collection ───────────────────────────────────────────────────────────
async function collectSampleUrls(page, limit) {
    const urls = new Set();
...
async function runBenchmark() {
    ...
    try {
        ...
        const cssFills = Object.fromEntries(TRACKED_FIELDS.map((f) => [f, 0]));
        const llmOnlyFills = Object.fromEntries(TRACKED_FIELDS.map((f) => [f, 0]));
        const hybridFills = Object.fromEntries(TRACKED_FIELDS.map((f) => [f, 0]));
        // ── Per-page extraction ──────────────────────────────────────────────────
        for (let i = 0; i < urls.length; i++) {
            ...
            try {
                ...
                const llmOnlyResult = Object.fromEntries(TRACKED_FIELDS.map((f) => [f, llmOnlyRaw.fields[f] ?? null]));
                ...
                if (missingFields.length > 0) {
                    ...
                    for (const f of missingFields)
                        missingSchemas[f] = FIELD_SCHEMAS[f];
                    const t2 = Date.now();
                ...
                pageResults.push({
                    ...
                    hybrid: { fieldsFound: hybridCount, fields: hybridMap, speedMs: hybridSpeedMs, tokensUsed: hybridTokens, error: hybridCount === 0 },
                });
                if ((i + 1) % 5 === 0 || i === urls.length - 1) {
                    const n = pageResults.length;
                    console.log(`[benchmark] ${i + 1}/${urls.length} — ` +
                ...
                const zeroFields = Object.fromEntries(TRACKED_FIELDS.map((f) => [f, false]));
                pageResults.push({
                    url,
        ...
        if (RUN_RESILIENCE && n > 0) {
            ...
            const llmOrigResult = Object.fromEntries(TRACKED_FIELDS.map((f) => [f, llmOrigRaw.fields[f] ?? null]));
            ...
            const llmModResult = Object.fromEntries(TRACKED_FIELDS.map((f) => [f, llmModRaw.fields[f] ?? null]));
            ...
            if (modMissing.length > 0) {
                const modSchemas = {};
                for (const f of modMissing)
                    modSchemas[f] = FIELD_SCHEMAS[f];
            ...
            const pct = (a, b) => b > 0 ? Math.round(((b - a) / b) * 100) : 0;
            resilienceData = {
            ...
            console.log(`[benchmark] Resilience — CSS drop: ${resilienceData.cssDropPct}%, ` +
                `LLM-only drop: ${resilienceData.llmOnlyDropPct}%, ` +
                `hybrid drop: ${resilienceData.hybridDropPct}%`);
        }
        // ── Aggregate metrics ────────────────────────────────────────────────────
        const avg = (fn) => Math.round((pageResults.reduce((s, r) => s + fn(r), 0) / n) * 100) / 100;
        const errorRate = (fn) => Math.round((pageResults.filter((r) => fn(r)).length / n) * 100);
        // Haiku blended estimate: $1/1M input + $5/1M output ≈ $3/1M blended
        const costUsd = (tokens) => Math.round(((tokens / 1_000_000) * 3.0) * 10_000) / 10_000;
        const fieldBreakdown = Object.fromEntries(TRACKED_FIELDS.map((f) => [
            f,
            {
        ...
        for (const [field, stats] of Object.entries(report.fieldBreakdown)) {
            console.log(`  ${field.padEnd(24)}  css: ${String(stats.cssFillRate).padStart(3)}%  llm: ${String(stats.llmOnlyFillRate).padStart(3)}%  hybrid: ${String(stats.hybridFillRate).padStart(3)}%`);
        }
        if (OUTPUT_PATH) {
...
//# sourceMappingURL=llm-extraction-benchmark.js.map