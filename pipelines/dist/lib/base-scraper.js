...
export class BaseScraper {
    ...
    sleep(ms) {
        return new Promise((resolve) => setTimeout(resolve, ms ?? this.config.requestDelayMs));
    }
        ...
        for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
            try {
                ...
                if (attempt === this.config.maxRetries)
                    throw err;
                console.warn(`[${this.config.pipelineName}][fetcher] Attempt ${attempt} failed for ${url}: ${err}. Retrying...`);
    ...
    async captureIntegrity(url, pipelineRunId) {
        try {
            ...
            console.warn(`[${this.config.pipelineName}][integrity] Failed to capture integrity for ${url}: ${err}`);
        }
    }
        ...
        return new Promise((resolve) => {
            ...
            const handler = (res) => {
                ...
                for (const key of CAPTURED_HEADERS) {
                    ...
                    if (val)
                        httpHeaders[key] = Array.isArray(val) ? val[0] : val;
                }
    ...
    async run() {
        ...
        try {
            ...
            console.log(`[${name}] Completed — new: ${stats.newCount}, updated: ${stats.updatedCount}, ` +
