...
export async function sendApiKeyQuotaWarningEmail(email: string, usedPercent: number): Promise<void> {
  await sendEmail({
    to: email,
...
export async function sendApiKeyExpiryWarningEmail(email: string, keyName: string, daysLeft: number): Promise<void> {
  await sendEmail({
    to: email,
